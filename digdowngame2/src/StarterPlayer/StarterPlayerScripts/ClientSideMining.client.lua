...
-- Get RemoteEvents and RemoteFunctions
...
-- Create deleter RemoteFunction if it doesn't exist
if not ReplicatedStorage:FindFirstChild("deleter") then
	local deleter = Instance.new("RemoteFunction")
...
local function handleMining(ore, tool)
	if ore and ore:IsA("BasePart") and ore:FindFirstChild("Durability") then
		...
		-- If ore is broken (durability reached 0), invoke the deleter
		if ore.Durability.Value <= tool.Damage.Value then
			deleter:InvokeServer(ore)
		end
	end
end
...
-- Connect to block collection
deleter.OnClientEvent:Connect(function(block)
	print("Collected block:", block.Name)
end)